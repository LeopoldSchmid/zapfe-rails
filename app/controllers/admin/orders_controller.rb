class Admin::OrdersController < Admin::BaseController
  before_action :set_order, only: %i[show update add_attachments add_note archive unarchive download_attachment]
  ...
  def show
    ...
    @activities = @order.activities.includes(:admin_user).order(created_at: :desc)
    ...
    @procurement_plans = @order.procurement_plans.includes(items: :supplier_offering).order(created_at: :desc)
  ...
  def update
    ...
    if @order.update(order_params)
      ...
      @procurement_plans = @order.procurement_plans.includes(items: :supplier_offering).order(created_at: :desc)
      @actual_time_entries = @order.time_entries.where(entry_type: "actual").includes(:admin_user).order(recorded_on: :desc, created_at: :desc)
      @checklists = @order.checklists.includes(:items).order(:section, :created_at)
      @checklist_templates = ChecklistTemplate.active.includes(:items).order(:resource_type, :section, :name)
