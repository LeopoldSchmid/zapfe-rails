class Admin::OffersController < Admin::BaseController
  before_action :set_offer, only: %i[show update finalize duplicate document send_mail resolve]
  ...
  def prepare_show
    ...
    @supplier_offerings = SupplierOffering.active.includes(:supplier, product_variant: :product).order("suppliers.name").references(:supplier)
    @supplier_offerings_by_variant = @supplier_offerings.group_by(&:product_variant_id).transform_values do |offerings|
      offerings.sort_by { |offering| offering.current_price&.purchase_price || BigDecimal("999999999") }
    end
