class Admin::ProcurementPlansController < Admin::BaseController
  ...
  before_action :set_plan, only: %i[update add_attachments download_attachment]
  ...
  def update
    if confirming_non_returnable_without_acknowledgement?
      return redirect_to(admin_order_path(@order), alert: "Nicht rückgabefähige Positionen müssen vor der Bestätigung bewusst bestätigt werden.")
    end
  ...
  def add_attachments
    ...
    unless attachments.all? { |attachment| attachment.size <= 25.megabytes && attachment.content_type.in?(allowed_types) }
      return redirect_to(admin_order_path(@order), alert: "Erlaubt sind PDF, JPEG, PNG und WebP bis 25 MB.")
    end

    @plan.attachments.attach(attachments)
    redirect_to admin_order_path(@order), notice: "Beschaffungsanlagen hinzugefügt."
  rescue ActiveRecord::RecordInvalid
    redirect_to admin_order_path(@order), alert: @plan.errors.full_messages.to_sentence
  end
